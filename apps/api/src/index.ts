...
export class App {
    ...
    async config(socketIO?: Server) {
        ...
        if (
            ...
            parseInt(process.env.NUMBER_OF_PROXIES) > 0
        )
        ...
        this.app.post(
            ...
            async (req: Request, res: Response) => {
                ...
                if (user.length === 0)
                    return res.status(404).send(`User ${updatedUser.username} not found`);
        ...
        this.app.post("/api/v1/user/uaa", async (req: Request, res: Response) => {
            ...
            return res
                .status(404)
                .send(`User ${newUserAuthenticationRequest.username} not found`);
        });
        ...
        this.app.post("/api/v1/user/new", async (req: Request, res: Response) => {
            ...
            if (!newUser?.userProfile?.emailAddress)
            ...
            if (existingUser)
                return res.status(400).send(`User ${newUser.username} already exists`);
        ...
        this.app.get("/api/v1/user/:id", async (req: Request, res: Response) => {
            const id = req.params.id;
            const existingUser = await this.AppDataSource.getRepository(
                User
            ).findOneBy({ id });
            if (!existingUser)
                return res.status(404).send(`User with id: ${id} not found`);
        ...
        this.app.post("/api/v1/company", async (req: Request, res: Response) => {
            ...
            const results = await this.AppDataSource.getRepository(Company).save(
                newCompany
            );
            return res.json(results);
        });
        ...
        this.app.delete(
            "/api/v1/company/:id",
            async (req: Request, res: Response) => {
                const results = await this.AppDataSource.getRepository(Company).delete(
                    req.params.id
                );
                return res.json(results);
            }
        ...
        this.app.get(
            "/api/v1/user-profile",
            async (req: Request, res: Response) => {
                const results = await this.AppDataSource.getRepository(
                    UserProfile
                ).find();
                return res.json(results);
            }
        ...
        this.app.get(
            "/api/v1/solicitation/agency/:agency",
            async (req: Request, res: Response) => {
                const solicitations =
                    await solicitationService.getSolicitationsByAgency(req.params.agency);
        ...
        this.app.get(
            "/api/v1/solicitation",
            async (req: Request, res: Response) => {
                const solicitations = await solicitationService.getAllSolicitations(
                    req,
                    res
                );
                return solicitations;
        ...
        this.app.get(
            "/api/v1/solicitation/:id",
            async (req: Request, res: Response) => {
                const solicitation = await solicitationService.getSolicitation(
                    req.params.id
                );
